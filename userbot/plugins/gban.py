# this plugin ported from HellBot to Mafia Userbot
...
HELLBOY = str(ALIVE_NAME) if ALIVE_NAME else "Mafia Userbot"
papa = borg.uid
...
async def gban(userbot):
    ...
    await hellbot.edit(f"`🔥Global Ban Iz Coming'💦.... Wait and watch nigga🚶`")
...
async def gunban(userbot):
    ...
    if user:
        if user.id == 1212368262:
            return await hellbot.edit("**Didn't , Your Father Teach You ? That You Cant Gban your creator Himanshu😑😑🖕**")
        try:
