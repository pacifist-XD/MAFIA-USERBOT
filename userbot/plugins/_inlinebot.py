...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@MAFIA_USERBOT":
            rev_text = query[::-1]
            veriler = button(0, sorted(CMD_HELP))
            result = await builder.article(
                f"Hey! Only use .help please",
                text=f"**Running Mafia Userbot**\n\n__Number of plugins installed__ :`{len(CMD_HELP)}`\n**page:** 1/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False,
            )
            ...
            result = builder.article(
                "@MAFIA_USERBOT",
                text="""**Hey! This is [MAFIA USERBOT](https://t.me/MAFIA_USERBOT) \nYou can know more about me from the links given below 👇**\nIf you want plugins info please type `mymafia`""",
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/MAFIA_USERBOT"),
                        custom.Button.url(
                            "⚡ GROUP ⚡", "https://t.me/MAFIABOT_SUPPORT"
                        ),
                    ],
                    [
                        custom.Button.url(
                            "✨ REPO ✨", "https://github.com/H1M4N5HU0P/MAFIA-USERBOT"),
                        custom.Button.url
                    (
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "Hoo gya aapka. Kabse tapar tapar dabae jaa rhe h. Khudka bna lo na agr chaiye to. © MAFIA USERBOT™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Legenday AF** [MAFIA USERBOT](https://t.me/MAFIA_USERBOT) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await delete_mafia(event,
              "⚜️Mafia Userbot Menu Provider Is now Closed⚜️\n\n         **[© MAFIA USERBOT™](t.me/MAFIA_USERBOT)**", 5, link_preview=False
            )
        else:
            mafia_alert = "Ho gya aapka? Kabse tapar tapar dabae jaa rhe h. Khudka bna lo na agr chaiye to. © MAFIA USERBOT™"
